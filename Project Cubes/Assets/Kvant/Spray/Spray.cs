...
namespace Kvant
{
    ...
    public partial class Spray : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Experimental (Made by nick)
            /*
            Ray ray = new Ray(this.transform.position, this.transform.forward);
            //Debug.DrawRay(this.transform.forward, this.transform.forward * _initialVelocity.z, Color.red);
            RaycastHit hit;
            if (Physics.Raycast(ray, out hit, _initialVelocity.z * _life))
            {
                float dist = Vector3.Distance(this.transform.position, hit.transform.position);
                _initialVelocity.z = (dist / 5.5f);
            }
            else
            {
                _initialVelocity.z = _originalLife;
            }
            */
            ...
            for (var i = 0; i < _positionBuffer2.height; i++)
            {
                ...
                Graphics.DrawMesh(mesh, matrix, material, 0, null, 
                    0, props, _castShadows, _receiveShadows);
